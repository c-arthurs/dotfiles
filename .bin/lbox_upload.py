...
if __name__ == "__main__":
    ...
    parser.add_argument("apikeypath", type=str, help="The path to the labelbox api key")

    args = parser.parse_args()
    savedir = join(str(args.path), "low level export/")
    print(f"\n\tpath: {str(args.path)} \n\tsavepath: {savedir}\n")
    lbox_upload.upload_specific_file(
        dataset_name=str(args.dataset_name),
        file_path=str(args.path),
        apikeypath=str(args.apikeypath),
    )
